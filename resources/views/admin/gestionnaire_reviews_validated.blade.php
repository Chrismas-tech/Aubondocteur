...
@section('content')
    <div class="jumbotron text-primary bg_photo_4 mb-0">
        <div id="review_validated">

            @if ($review_status_validated->isEmpty())
            ...
            @else
                ...
                <div class="d-flex justify-content-between flex-wrap p-4">
                    ...
                    <table class="table table-striped  bg-white table-white border">
                        <thead>
                            <tr>
                                <th class="bg-success p_texte_3 text-white p-3 border">Ajouté par l'utilisateur
                                </th>
                                <th class="bg-success p_texte_3 text-white p-3 border">Médecin</th>
                                <th class="bg-success p_texte_3 text-white p-3 border">Date du Rendez-vous</th>
                                <th class="bg-success p_texte_3 text-white p-3 border">Commentaire</th>
                                <th class="bg-success p_texte_3 text-white p-3 border">Statut de la validation</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($review_status_validated as $review)
                                <tr>
                                    ...
                                    <td class="">
                                        <div class="d-flex justify-content-center">
                                            <form
                                                action="{{ route('review_validate_to_waiting', ['review' => $review->id, 'medecin_id' => $review->medecin_id, 'user_id' => $review->user->id]) }}"
                                                method="POST">
                                                @csrf
                                                @method('PATCH')
                                                <div>
                                                    <button type="submit"
                                                        class="btn btn-warning p_texte_3 text-dark mr-3"><img
                                                            src="{{ asset('img/sablier.png') }}" alt=""
                                                            class="icon_avis_button mr-1">Remettre en attente</button>
                                                </div>
                                            </form>
                                            <form
                                                action="{{ route('review_validate_to_refuse', ['review' => $review->id, 'medecin_id' => $review->medecin_id, 'user_id' => $review->user->id]) }}"
                                                method="POST">
                                                @csrf
                                                @method('PATCH')
                                                <div>
