<div class="flex">
    ...
    <div class="search_bar_flex_basis_1">
        <form action="{{ route('liste_medecins') }}" method="get">
            ...
            <select type="text" name="select_accueil" id="select_accueil"
                class="form-control @error('select_accueil') is-invalid @enderror">
...
@error('select_accueil')
    <div class="d-flex">
        <p class="alert alert-warning text-dark m-0 p_texte_2 lora media_accueil_error_1 mt-2 mt-sm-2 mt-md-3">Vous devez choisir la
            spécialité du
            médecin !
        </p>
    </div>
...
@error('input_search_accueil')
    <div class="d-flex">
        <p class="alert alert-warning text-dark m-0 p_texte_2 lora media_accueil_error_1 mt-2 mt-sm-2 mt-md-3">Vous n'avez pas choisi de
            localité !</p>
    </div>
@enderror
...
<div>
    @if ($count_medecins == 0)
        <p class="p_texte_28 lora text-dark m-0 p-0 text-center media_accueil_error_2 mt-2 mt-sm-2 mt-md-3">Il n'y a pas de
            résultats pour
            votre recherche :(
        </p>

    @elseif ($count_medecins == 1)
        <p class="p_texte_28 lora text-dark m-0 p-0 text-center media_accueil_error_2 mt-2 mt-sm-2 mt-md-3">Il y a 1 résultat pour
            votre
            recherche : <br>
    ...
    @elseif($count_medecins > 1)
        <p class="p_texte_28 lora text-dark m-0 p-0 text-center media_accueil_error_2 mt-2 mt-md-3">Il y a {{ $count_medecins }}
...
<script>
    $('document').ready(function() {
        ...
        $('document').ready(function() {
            ...
            $(document).on('click', '.li_result_search_click', function() {
    ...
    $('document').ready(function() {
        if (window.innerWidth < 1100) {
            ...
            $('#media_queries_option').html('Spécialité du médecin');
            $('#input_search_accueil').attr('placeholder',
                'Recherchez une ville ou un code postal');
        }
